...
CREATE OR REPLACE FUNCTION by_phone(IN search_phone character varying)
  RETURNS TABLE(curr_status statuses, order_num integer, o_date date, description text, contact_name character varying, 
	...
	contact_phone character varying, s_date date) AS
$BODY$ 
		
	BEGIN

	RETURN QUERY
	select status.curr_status, rik_order.order_num, rik_order.o_date, rik_order.description, supplier.contact_name, 
	...
	ORDER BY status.s_date DESC LIMIT 1;
